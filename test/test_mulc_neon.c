...
int main(void)
{
    ...
    mn_vec2i_t cst_v2i = { 2, -3 };
    mn_vec3f_t cst_v3f = { 0.1f, 0.2f, 0.3f };
    mn_vec3i_t cst_v3i = { 2, 3, 4 };
    mn_vec4f_t cst_v4f = { 1.0f, -2.0f, 3.0f, -4.0f };
    mn_vec4i_t cst_v4i = { 2, 3, 4, 5 };
    ...
    // ==== reference (C) ====
    mn_mulc_float_c(ref_f, src_f, cst_f, count);
    mn_mulc_int32_c(ref_i, src_i, cst_i, count);
    mn_mulc_vec2f_c(ref_v2f, src_v2f, &cst_v2f, count);
    mn_mulc_vec2i_c(ref_v2i, src_v2i, &cst_v2i, count);
    mn_mulc_vec3f_c(ref_v3f, src_v3f, &cst_v3f, count);
    mn_mulc_vec3i_c(ref_v3i, src_v3i, &cst_v3i, count);
    mn_mulc_vec4f_c(ref_v4f, src_v4f, &cst_v4f, count);
    mn_mulc_vec4i_c(ref_v4i, src_v4i, &cst_v4i, count);

    // ==== NEON ====
    mn_mulc_float_neon(dst_f, src_f, cst_f, count);
    mn_mulc_int32_neon(dst_i, src_i, cst_i, count);
    mn_mulc_vec2f_neon(dst_v2f, src_v2f, &cst_v2f, count);
    mn_mulc_vec2i_neon(dst_v2i, src_v2i, &cst_v2i, count);
    mn_mulc_vec3f_neon(dst_v3f, src_v3f, &cst_v3f, count);
    mn_mulc_vec3i_neon(dst_v3i, src_v3i, &cst_v3i, count);
    mn_mulc_vec4f_neon(dst_v4f, src_v4f, &cst_v4f, count);
    mn_mulc_vec4i_neon(dst_v4i, src_v4i, &cst_v4i, count);

    // ==== validate ====
    for (int i = 0; i < count; i++) {
        if (dst_f[i] != ref_f[i] || dst_i[i] != ref_i[i]) {
            printf("Flat mulc test failed at %d\n", i); return 1;
        }
        if (dst_v2f[i].x != ref_v2f[i].x || dst_v2f[i].y != ref_v2f[i].y ||
            dst_v2i[i].x != ref_v2i[i].x || dst_v2i[i].y != ref_v2i[i].y) {
            printf("Vec2 mulc test failed at %d\n", i); return 1;
        }
        if (dst_v3f[i].x != ref_v3f[i].x || dst_v3f[i].y != ref_v3f[i].y || dst_v3f[i].z != ref_v3f[i].z ||
            ...
            printf("Vec3 mulc test failed at %d\n", i); return 1;
        }
        if (dst_v4f[i].x != ref_v4f[i].x || dst_v4f[i].y != ref_v4f[i].y ||
            ...
            printf("Vec4 mulc test failed at %d\n", i); return 1;
        }
    }

    printf("All mulc tests passed!\n");
