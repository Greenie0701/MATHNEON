...
int main(void)
{
    ...
    // ==== reference (C) ====
    mn_mul_float_c(ref_f, src1_f, src2_f, count);
    mn_mul_int32_c(ref_i, src1_i, src2_i, count);
    mn_mul_vec2f_c(ref_v2f, src1_v2f, src2_v2f, count);
    mn_mul_vec2i_c(ref_v2i, src1_v2i, src2_v2i, count);
    mn_mul_vec3f_c(ref_v3f, src1_v3f, src2_v3f, count);
    mn_mul_vec3i_c(ref_v3i, src1_v3i, src2_v3i, count);
    mn_mul_vec4f_c(ref_v4f, src1_v4f, src2_v4f, count);
    mn_mul_vec4i_c(ref_v4i, src1_v4i, src2_v4i, count);

    // ==== NEON ====
    mn_mul_float_neon(dst_f, src1_f, src2_f, count);
    mn_mul_int32_neon(dst_i, src1_i, src2_i, count);
    mn_mul_vec2f_neon(dst_v2f, src1_v2f, src2_v2f, count);
    mn_mul_vec2i_neon(dst_v2i, src1_v2i, src2_v2i, count);
    mn_mul_vec3f_neon(dst_v3f, src1_v3f, src2_v3f, count);
    mn_mul_vec3i_neon(dst_v3i, src1_v3i, src2_v3i, count);
    mn_mul_vec4f_neon(dst_v4f, src1_v4f, src2_v4f, count);
    mn_mul_vec4i_neon(dst_v4i, src1_v4i, src2_v4i, count);

    // ==== validate ====
    for (int i = 0; i < count; i++) {
        if (dst_f[i] != ref_f[i] || dst_i[i] != ref_i[i]) {
            printf("Flat mul test failed at %d\n", i); return 1;
        }
        if (dst_v2f[i].x != ref_v2f[i].x || dst_v2f[i].y != ref_v2f[i].y ||
            dst_v2i[i].x != ref_v2i[i].x || dst_v2i[i].y != ref_v2i[i].y) {
            printf("Vec2 mul test failed at %d\n", i); return 1;
        }
        if (dst_v3f[i].x != ref_v3f[i].x || dst_v3f[i].y != ref_v3f[i].y || dst_v3f[i].z != ref_v3f[i].z ||
            ...
            printf("Vec3 mul test failed at %d\n", i); return 1;
        }
        if (dst_v4f[i].x != ref_v4f[i].x || dst_v4f[i].y != ref_v4f[i].y ||
            ...
            printf("Vec4 mul test failed at %d\n", i); return 1;
        }
    }
